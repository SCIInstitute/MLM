...
for ii in range(len(Basket_pos)):
	if Basket_pos_t[ii] >= tstart:
		time = int((Basket_pos_t[ii]-tstart)/tres)
		location = int(Basket_pos[ii]/sres)		
...
for ii in range(len(Basket_pos)):
	if Basket_pos_t[ii] >= tstart:
		time = int((Basket_pos_t[ii]-tstart)/tresr)
		location = int(Basket_pos[ii]/sresr)		
...
for ii in range(len(Basket_pos)):
	if Basket_pos_t[ii] >= tstart:
		time = int((Basket_pos_t[ii]-tstart)/tres2D)
		location = int(Basket_pos[ii]/sres2D)		
...
plt.ylim(15, np.max(np.mean(log10tvs,0)))
plt.xlabel('Temporal Frequency (Hz)')
...
plt.figure(4)
plt.plot(sfrequency, np.mean(log10, 0), '.b', markersize=6)
